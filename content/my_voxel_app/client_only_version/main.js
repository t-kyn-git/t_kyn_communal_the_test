...
function updateRollOverMesh() {
    ...
    if (intersects.length > 0) {
        ...
        let newPos;
        if (targetedBlock) {
            const blockPos = new THREE.Vector3().copy(targetedBlock.position).floor();
            newPos = blockPos.add(intersect.face.normal);
        } else {
            newPos = new THREE.Vector3().copy(intersect.point).add(intersect.face.normal);
        }
...
function isBlockAt(position) {
    for (const obj of objects) {
        if (obj.isMesh && obj !== plane) {
            const blockPos = new THREE.Vector3().copy(obj.position).floor();
...
function createBlockSelector() {
    ...
    for (const type in BLOCK_TYPES) {
        ...
        item.innerHTML = `<input type="radio" name="blocktype" id="type-${type}" value="${type}"><label for="type-${type}"><img src="${BLOCK_TYPES[type].texture}" class="block-preview"> ${type}</label>`;
